...
## Generating or Editing Lecture Notes

This repo's actual content deliverable is the Bhakti Shastri lecture notes under `outputs/` and
the Fumadocs site in `web/` that renders them — the `agents-cli`/ADK guidance below is for the
`app/` prototype agent that extracts and grounds those notes.

Before generating a new lecture's notes, restructuring an existing note, or touching the
`Shloka`/verse-injection pipeline, read
**[.github/instructions/notes-generation.instructions.md](.github/instructions/notes-generation.instructions.md)**.
